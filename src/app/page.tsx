...
export default function Home() {
  ...
  const [results, setResults] = useState<SearchResult>({ items: [], suggestions: [] });
      ...
      <div className="max-w-2xl mx-auto space-y-12">
        ...
        <header className="text-center space-y-4">
          ...
          <p className="text-lg text-muted-foreground max-w-md mx-auto">
            Your intelligent product and price discovery engine. Search for anything and get instant results.
          </p>
        </header>
        ...
        <section className="relative">
          <form onSubmit={handleSearch} className="relative group">
            <div className="relative">
              ...
              <Input
                type="text"
                placeholder="Search for items like 'iPhone', 'Vacuum', 'Macbook'..."
                value={query}
        ...
        <section className="space-y-6">
          {!hasSearched && !isPending && (
            <div className="text-center py-20 opacity-40">
              ...
              <p className="text-xl font-medium">Start typing to find products</p>
            </div>
          )}
          ...
          <SearchResults 
            ...
            onSuggestionClick={(s) => handleSearch(undefined, s)}
            isLoading={isPending}
          />
          ...
          {hasSearched && results.items.length > 0 && !isPending && (
            <div className="flex items-center justify-center gap-2 pt-8 text-sm text-muted-foreground">
              ...
              <p>Found {results.items.length} matching result{results.items.length > 1 ? 's' : ''}</p>
            </div>
          )}
        </section>
      </div>
